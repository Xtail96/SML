#include "device_arduino_adapter.h"

ArduinoDeviceAdapter::ArduinoDeviceAdapter(QString portName, QObject *parent) :
    QObject(parent),
...
ArduinoDeviceAdapter::~ArduinoDeviceAdapter()
{
...
void ArduinoDeviceAdapter::loadSettings()
{
    try
    {
        ...
        m_currentState = new DeviceControllerState(sensorsPackageSize, devicesPackageSize);
        m_previousState = new DeviceControllerState(sensorsPackageSize, devicesPackageSize);
    ...
    catch(std::invalid_argument e)
    {
        ...
        m_currentState = new DeviceControllerState();
        m_previousState = new DeviceControllerState();
    }
}

bool ArduinoDeviceAdapter::isStateChanged()
{
...
void ArduinoDeviceAdapter::printState()
{
    qDebug() << "New State";
    qDebug() << "Sensors:";
...
void ArduinoDeviceAdapter::openPort(const QSerialPortInfo &info)
{
...
void ArduinoDeviceAdapter::onQSerialPort_ReadyRead()
{
    qDebug() << "start read";
    QByteArray data;
...
void ArduinoDeviceAdapter::onQSerialPort_ReadFromPort(QByteArray received)
{
...
void ArduinoDeviceAdapter::switchDevice(byte_array data)
{
...
void ArduinoDeviceAdapter::switchDevice(size_t index, QString target, QString type)
{
    byte indexByte = 0xff;
    switch (index) {
    case 0:
    ...
    if(type.toLower() == "spindel")
    {
        if(target.toLower() == "on")
        {
            ...
            qDebug() << "DeviceArduinoAdapter::switchDevice: on"<< message;
        ...
        else
        {
            if(target.toLower() == "off")
            {
                ...
                message.push_back(indexByte);
                message.push_back(paramsByte);
                qDebug() << "DeviceArduinoAdapter::switchDevice: off"<< message;
                m_serial->write(message);
            }
        }
    }
...
void ArduinoDeviceAdapter::getCurrentState()
{
    QByteArray message;
    message.push_back(GET_CURRENT_STATE);
    qDebug() << "DeviceArduinoAdapter::getCurrentState:" << message;
    m_serial->write(message);
}

void ArduinoDeviceAdapter::sendStateToServer(DeviceControllerState *state)
{
    ...
    deviceControllerState["sensorsState"] = sensorsState;
    deviceControllerState["devicesState"] = devicesState;
...
void ArduinoDeviceAdapter::onWebSocketHandler_BinaryMessageReceived(QByteArray message)
{
    ...
    {
        qDebug() << "DeviceArduinoAdapter::onWebSocketHandler_BinaryMessageReceived: message ignored" << messageString;
    }
}

void ArduinoDeviceAdapter::sendTestPackageToServer()
{
    this->sendStateToServer(m_currentState);
}

void ArduinoDeviceAdapter::onWebSocketHandler_Connected()
{
    qDebug() << "Web socket is connected";
    this->sendTestPackageToServer();
}

void ArduinoDeviceAdapter::onWebSocketHandler_Disconnected(QWebSocketProtocol::CloseCode code, QString message)
{
